    ...
    <head>
        <meta charset="UTF-8">
            @php
                ...
                $sidebarLogo = ($currentUser && !$currentUser->hasRole('superadmin') && $companyLogo)
                    ? $companyLogo
                    : $systemLogo;

                $sidebarName = ($currentUser && !$currentUser->hasRole('superadmin') && $currentUser->company)
    ...
    <div x-data="{ sidebarOpen: localStorage.getItem('sidebarOpen') !== 'false' }"
        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
            ...
            <div class="mb-8 w-full text-center">
                <div class="flex flex-col items-center justify-center w-full">
                    @if ($sidebarLogo)
                        <img src="{{ asset('storage/' . $sidebarLogo) }}"
                            style="width:70px; height:70px; max-width:70px; max-height:70px; object-fit:cover; display:block; border-radius:9999px; padding:0px; background:white;">
                    @else
                    ...
                    <span x-show="sidebarOpen"
                        class="font-bold text-white text-center mt-2 block w-full">
                        {{ $sidebarName }}
                    </span>
                </div>
            </div>
                ...
                <div x-show="sidebarOpen" class="mt-2 mb-2 px-3 text-xs font-bold text-gray-400 uppercase tracking-wider">
                    Operaciones
                </div>

                <a href="{{ route('dashboard') }}"
                :class="sidebarOpen ? 'justify-start gap-3 px-3' : 'justify-center px-0'"
                class="flex items-center py-2 rounded hover:bg-gray-800">
                    <span>&#128202;</span>
                ...
                @can('projects.import')
                    ...
                    class="flex items-center py-2 rounded hover:bg-gray-800">
                        <span>&#128229;</span>
                        <span x-show="sidebarOpen">Importaciones KD</span>
                    </a>
                @endcan
                ...
                @can('material-prices.view')
                    <a href="{{ route('material-prices.index') }}"
                    :class="sidebarOpen ? 'justify-start gap-3 px-3' : 'justify-center px-0'"
                    class="flex items-center py-2 rounded hover:bg-gray-800">
                        <span>&#128176;</span>
                        <span x-show="sidebarOpen">Materiales</span>
                    </a>
                @endcan

                @can('module-prices.view')
                    <a href="{{ route('module-prices.index') }}"
                    :class="sidebarOpen ? 'justify-start gap-3 px-3' : 'justify-center px-0'"
                    class="flex items-center py-2 rounded hover:bg-gray-800">
                        <span>&#128230;</span>
                        <span x-show="sidebarOpen">Modulos</span>
                    </a>
                @endcan

                @if(
                    auth()->user()->can('company-settings.view') ||
                    auth()->user()->can('companies.view') ||
                    auth()->user()->can('users.view') ||
                    auth()->user()->can('roles.view') ||
                    auth()->user()->can('settings.view')
                )
                    <div x-show="sidebarOpen" class="mt-6 mb-2 px-3 text-xs font-bold text-gray-400 uppercase tracking-wider">
                        Administracion
                    </div>
                @endif
                ...
                @can('company-settings.view')
                    ...
                    class="flex items-center py-2 rounded hover:bg-gray-800">
                        <span>&#127970;</span>
                        <span x-show="sidebarOpen">Mi empresa</span>
                    </a>
                @endcan
            ...
            <header class="bg-white shadow px-6 py-4 flex justify-between items-center">
                <div class="flex items-center">
                    <button @click="sidebarOpen = !sidebarOpen"
                            class="mr-3 p-2 rounded bg-gray-900 hover:bg-gray-800 text-white shadow">
                        &#9776;
                    </button>

                    <span class="font-semibold text-gray-700">
                        {{ strtoupper(auth()->user()->company->name ?? $systemName) }}
                    </span>
                </div>
    ...
    <div x-data="{ userMenuOpen: false }" class="relative">
        ...
        <button @click="userMenuOpen = !userMenuOpen"
            ...
            <div class="text-left leading-tight">
                ...
                <div class="mt-0.5 text-[11px] font-semibold uppercase tracking-wide text-indigo-600">
