...
<div class="bg-white shadow rounded-lg p-6">
    <form method="GET" action="{{ route('users.index') }}" class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="flex items-end gap-2">
            ...
            <a href="{{ route('users.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200">
                Limpiar
            </a>
        </div>
    </form>
                        ...
                        <td class="px-4 py-3 text-sm text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                @if (!$user->hasRole('superadmin'))
                                    @can('users.toggle')
                                        <form method="POST" action="{{ route('users.toggle', $user->id) }}" onsubmit="return confirm('¿Confirmas cambiar el estado de este usuario?');">
                                            @csrf
                                            <button type="submit" class="px-3 py-1 bg-gray-100 text-gray-800 rounded-md text-xs hover:bg-gray-200">
                                    ...
                                    @can('users.reset')
                                        <form method="POST" action="{{ route('users.reset', $user->id) }}" onsubmit="return confirm('¿Confirmas resetear la clave de este usuario?');">
                                            @csrf
                                            <button type="submit" class="px-3 py-1 bg-indigo-100 text-indigo-800 rounded-md text-xs hover:bg-indigo-200">
                                                Reset
                                            </button>
                                        </form>
                                    @endcan
